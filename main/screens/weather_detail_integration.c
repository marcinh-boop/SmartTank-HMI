/*
 * Moduł weather_detail_integration.c należy do warstwy głównej programu SmartTank.
...
static lv_obj_t *s_weather_card;
...
static bool find_dashboard_and_weather_card(
    lv_obj_t *screen,
    lv_obj_t **dashboard_content,
    lv_obj_t **weather_card)
{
    if (screen == NULL || dashboard_content == NULL || weather_card == NULL) {
        return false;
    }
    ...
    for (uint32_t i = 0U; i < screen_child_count; i++) {
        ...
        for (uint32_t j = 0U; j < layer_child_count; j++) {
            ...
            card_count++;
            if (lv_obj_get_x(candidate_card) >= 500) {
                right_card = candidate_card;
            }
        }

        if (card_count >= 3U && right_card != NULL) {
            *dashboard_content = candidate_layer;
            *weather_card = right_card;
            return true;
        }
    }
...
static void weather_card_event_cb(lv_event_t *event)
{
    (void)event;
    ...
    smarttank_state_t state;
    app_model_get_snapshot(&state);
    screen_weather_detail_begin(&state);
    ...
    top_status_bar_set_page_title("POGODA");
}
...
bool weather_detail_integration_attach(lv_obj_t *screen)
{
    if (screen == NULL) {
        return false;
    }
    ...
    lv_obj_t *weather_card = NULL;
    if (!find_dashboard_and_weather_card(screen, &dashboard_content, &weather_card)) {
        return false;
    }
    ...
    s_weather_card = weather_card;
    s_detail_content = screen_weather_detail_create(screen, detail_back_cb);
    if (s_detail_content == NULL) {
        s_screen = NULL;
    ...
    lv_obj_set_style_bg_color(
        s_weather_card,
        lv_color_hex(0x3A3218),
        LV_PART_MAIN | LV_STATE_PRESSED
    );
    lv_obj_add_event_cb(
        s_weather_card,
        weather_card_event_cb,
        LV_EVENT_RELEASED,
        NULL
    );
