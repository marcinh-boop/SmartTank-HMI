...
static void open_dashboard_async(void *user_data)
{
    (void)user_data;
...
static bool history_nav_change(bottom_nav_page_t page)
{
    if (page == NAV_DASHBOARD) {
        lv_async_call(open_dashboard_async, NULL);
        return true;
    }
...
void screen_history_create(void)
{
    ...
    lv_obj_t *tank_panel = create_panel(screen, 20, 370);
    create_chart(
        tank_panel,
        "SZAMBO  72%",
    ...
    lv_obj_t *well_panel = create_panel(screen, 410, 370);
    create_chart(
        well_panel,
        "STUDNIA  2.81 m",
