...
static void open_history_async(void *user_data)
{
    (void)user_data;
...
{
    if (page == NAV_HISTORY) {
        lv_async_call(open_history_async, NULL);
        return true;
    }
...
void screen_dashboard_create(void)
{
    ...
    lv_obj_t *top = create_bar(screen, LV_ALIGN_TOP_MID, 8);
    ...
    lv_obj_t *tank_card = create_card(screen, lv_color_hex(0x39D12F));
    ...
    lv_obj_t *well_card = create_card(screen, lv_color_hex(0x2EA8FF));
    ...
    lv_obj_t *weather_card = create_card(screen, lv_color_hex(0xFFC247));
    ...
    lv_obj_t *bottom = create_bar(screen, LV_ALIGN_BOTTOM_MID, -8);
